...
const HorariosProfessor = (props) => {
  ...
  async function infoProfessor() {
    ...
    try {
      await axios
        .get("http://" + ipv4.ip + ":3000/profInfo")
  ...
  const listTurmasByEscolaEnsino = async () => {
    ...
    for (const element in listTurmas) {
      if (
        infoProf["ensino"] == listTurmas[element]["ensino"] &&
        infoProf["escola"] == listTurmas[element]["escola"]
      ) {
  ...
  async function generateValues() {
    var x = listTurmasByEscolaEnsino();
    ...
    if (currentValue && currentValue != "Selecione o professor: ") {
      ...
      Alert.alert(
        "Professor não selecionado!",
        "Por favor, selecione um professor para cadastrar seus horários.",
        [{ text: "Fechar" }]
      );
    }
  ...
  function whoIsAvailable(dia, horario) {
    ...
    if (listTurmas[randomTurma]["horarios"][dia][horario] == "")
      return turmas[randomTurma];
    else return "";
  }
  ...
  async function postHorariosProf(h) {
    try {
      await axios
        .post("http://" + ipv4.ip + ":3000/horario-professor", {
          horarios: h,
          nome: currentValue,
        })
        .then(function (response) {
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
        ...
        <View style={{paddingLeft:60,paddingRight:60, marginBottom:15}}>
            <Text>* Toque duas vezes o botão 'Gerar Horários' para gerar um horário válido para o professor selecionado.</Text>
        </View>
