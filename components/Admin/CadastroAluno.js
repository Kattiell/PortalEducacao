...
export default function CadastroAluno() {
  ...
  const [currentValueEscola, setCurrentValueEscola] = useState("Selecione a escola");
  ...
  const listTurma = async () => {
    ...
    var list = [];
    for (let turma in x) {
      list.push(x[turma]["numeroTurma"] + " (" + x[turma]["escola"] + ") ");
    }
    setListTurmas(list);
  };
  ...
  function showAlertErro(listErrors) {
    ...
    Alert.alert("Ocorreu um Erro!", erros, [
      {
        text: "Fechar",
        style: "color:'#FFFFFF'",
      },
