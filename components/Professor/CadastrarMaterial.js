...
const CadastrarMaterial = (props) => {
  ...
  const generateBlob = async (blobFile) => {
    let idx = 0;
    let ocurrence = 0;
    while (ocurrence < 6) {
      idx = blobFile.indexOf("/", idx + 1);
      if (idx != -1) ocurrence++;
    }
    let newvalue = blobFile.slice(idx + 1, blobFile.length);
    var r = await fetch(newvalue);
    var b = await r.blob();
    return b;
  };
    ...
    <View style={styles.container}>
      ...
      <DialogBox width={415} height={415}>
        <View style={styles.contentbx}>
          <Text style={styles.inputTextName}>Postar material de estudo</Text>
            ...
            onPress={async () => {
              if (edit == false) {
                ...
                const sotrageRef = ref(
                  storage,
                  `Materiais de estudo/${escola}/${numeroTurma}/${selectedDisciplina}/${fileName}`
                );
                  ...
                  () => {
                    getDownloadURL(uploadTask.snapshot.ref).then(
                      (downloadURL) => {
                        ...
                        navigation.navigate("Postar Conteudo", {
                          canUpdate: true,
                        });
