...
const HorariosProfessor = (props) =>{
    ...
    const putTurmaData = async() =>{
        ...
        for(let i = 0; i < n; i++){
            ...
            for(const i in listTurmas){
                for(const dia in listTurmas[i]){
                    for(const horario in listTurmas[i][dia]){
                        if(listTurmas[i][dia][horario] == '' || listTurmas[i][dia][horario]){
                            horarios[dia][horario] = currentValue;
                        }
                    }
                }
            }

            const turmacolec = await db.collection('turmas').get();
            const t = await turmacolec.where('numeroTurma','==',turmas[i]).get();
            const res = await t.doc('horarios').set(horarios);
