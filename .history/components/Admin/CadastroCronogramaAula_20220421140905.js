...
const HorariosProfessor = (props) =>{
    ...
    function generateValues(){
        ...
        if(currentValue && currentValue != 'Selecione o professor: '){
            // for(const t in listTurmas){
            //     for(const dia in listTurmas[t]["horarios"]){
            //         for(const horario in listTurmas[t]["horarios"][dia]){
            //             let n = getRandomIntInclusive(0,turmas.length-1);
            //             currentHorarios[dia][horario] = turmas[n];
            //         }
            //     }
            // }

            for(dia in currentHorarios){
                for(horario in currentHorarios[dia]){
                    let n = getRandomIntInclusive(0,turmas.length-1);
                    currentHorarios[dia][horario] = turmas[n];
                }
            }
