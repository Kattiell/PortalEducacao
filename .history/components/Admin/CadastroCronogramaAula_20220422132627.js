    ...
    const putTurmaData = async() =>{
       ...
       for(let i = 0; i < listTurmas.length; i++){
            ...
            for(const t in listTurmas){
                for(const h in listTurmas[t]){
                    for(const dia in listTurmas[t][h]){
                        console.log("Horário de "+ dia +": \n");
                        for(const horario in listTurmas[t][h][dia]){
                            horarios[dia][horario] = listTurmas[t][h][dia];
                        }
